...
vi.mock('@huggingface/inference', () => ({
  HfInference: vi.fn().mockImplementation(function () {
    return {
      imageToVideo: vi.fn().mockResolvedValue(new Blob(['fake-video'], { type: 'video/mp4' })),
    };
  }),
}));
...
async function importRoute() {
  return await import('@/app/api/hf/image-to-video/route');
}
...
function createRequest(formFields: Record<string, string | Blob>, token?: string) {
  ...
  return new NextRequest('http://localhost/api/hf/image-to-video', {
    method: 'POST',
    body: formData,
    headers,
  });
...
describe('POST /api/hf/image-to-video', () => {
  ...
  it('returns 400 when no image provided', async () => {
    const { POST } = await importRoute();
    const req = createRequest({ prompt: 'animate this' }, 'hf_token123');
  ...
  it('returns video/mp4 on success', async () => {
    const { POST } = await importRoute();
    const req = createRequest(
      {
        prompt: 'make it move',
  ...
  it('works without prompt (image-only)', async () => {
    const { POST } = await importRoute();
    const req = createRequest(
      { image: new Blob(['img'], { type: 'image/png' }) },
      'hf_token123'
  ...
  it('exports maxDuration of 300', async () => {
  ...
  it('returns 500 when HF SDK throws', async () => {
    ...
    (HfInference as unknown as ReturnType<typeof vi.fn>).mockImplementationOnce(function () {
      return {
        imageToVideo: vi.fn().mockRejectedValue(new Error('Generation failed')),
      };
    });
